...
def analyze_playlist(request):
    try:
        ...
        #   - if yes skip ML process for those songs and assign them to user pool if required
        #   - If no then go to ML process and add new songs to ClassifiedSongs table and user pool
        ...
        if (not is_empty) and (new_tracks_df is not None):
            ...
            if existing_tracks is not None:
                ...
                return_data = {
                    "error": "0",
                    "message": "Added songs to your pool successfully"
        ...
        return_data = {
            "error": "ANZ001",
            "message": "An error has occured in the Moodify process - Please try again later"
        }
...
def generate_playlist(request):
    try:
        ...
        # Select user_songs from User  where user_id
        # values_list() returns selected values in a list
        user_tracks = User.objects.values_list('user_songs', flat=True).filter(user_id=userid)[0]

        if len(user_tracks) != 0:
            mood_tracks = ClassifiedSong.objects.values_list('song_id', flat=True).filter(song_id__in=user_tracks,
                                                                                          mood=user_mood)

            if len(mood_tracks) != 0:
                # Get mood playlist index - add one in case it's 0
                playlist_no = MoodPlaylist.objects.filter(user_id=userid, mood=user_mood).count() + 1

                # If there is proceed to create the playlist
                new_playlist_id = spotify.create_playlist(userid, user_mood, mood_tracks, playlist_no, auth)
        ...
        return_data = {
            "error": "GEN001",
            "message": "Playlist could not be created. An error occured"
        }
